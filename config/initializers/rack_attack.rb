...
class Rack::Attack
  ...
  throttle('throttle_unauthenticated_api', limit: 1_500, period: 5.minutes) do |req|
    req.throttleable_remote_ip if req.api_request? && req.unauthenticated?
  end

  throttle('throttle_api_media', limit: 1_500, period: 30.minutes) do |req|
    req.authenticated_user_id if req.post? && req.path.match?(%r{\A/api/v\d+/media\z}i)
  end

  throttle('throttle_media_proxy', limit: 1_500, period: 10.minutes) do |req|
    req.throttleable_remote_ip if req.path.start_with?('/media_proxy')
  end

  throttle('throttle_api_sign_up', limit: 1_500, period: 30.minutes) do |req|
  ...
  throttle('throttle_unauthenticated_paging', limit: 1_500, period: 15.minutes) do |req|
